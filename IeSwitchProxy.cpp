...
STDAPI DllGetClassObject(REFCLSID rclsid, REFIID riid, LPVOID *ppReturn)
{
	...
	*ppReturn = NULL;
	...
	if(IsEqualCLSID(rclsid, CLSID_IeSwitchProxyBand))
	{
...
STDAPI DllRegisterServer()
{
	...
	try
	{
		...
		if(StringFromCLSID(CLSID_IeSwitchProxyBand, &pClsidStr) != S_OK)
		{
		...
		wchar_t *hKeyValue = L"IeSwitchProxy";
		if(RegSetValueEx(hKeyBand,
						 NULL,
						 0,
						 REG_SZ,
						 reinterpret_cast<const BYTE *>(hKeyValue),
						 14 * sizeof(wchar_t)) != ERROR_SUCCESS)
		{
